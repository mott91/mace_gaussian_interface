...
def create_final_simplified_workflow(prs):
    ...
    # Flowchart - centered with better alignment
    flow_box = slide.shapes.add_textbox(Inches(0.5), Inches(0.9), Inches(9.0), Inches(6.4))
    tf = flow_box.text_frame
    tf.word_wrap = False
    ...
    lines = [
        ...
        ("│ ──────────────────────────────────────────────────────────────────────────────────────── │", TEXT),
        ("│  ┌──────────┐  ┌───────────┐  ┌─────────────┐  ┌───────────┐  ┌────────────┐  ┌──────┐│", ACCENT),
        ("│  │ load_all │->│ find_dft_ │->│ eigenvector │->│ kde_      │->│ calculate_ │->│ plot ││", ACCENT),
        ("│  │ results()│  │ baseline()│  │ matching()  │  │ broaden() │  │ metrics()  │  │ regr.││", TEXT),
        ("│  └──────────┘  └───────────┘  └─────────────┘  └───────────┘  └────────────┘  └──────┘│", ACCENT),
        ("└──────────────────────────────────────────────────────────────────────────────────────────┘", ACCENT),
        ("                                        │", DIM),
        ("                                        ▼", DIM),
