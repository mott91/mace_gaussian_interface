...
def create_side_by_side_zmq_diagram(prs):
    ...
    diagram_lines = [
        ("┌─────────────────────┐        ┌─────────────────────┐", ACCENT),
        ("│  Main Process       │◀──────▶│  Gaussian 16        │", ACCENT),
        ("│  (Python)           │  ZMQ   │  (Fortran)          │", ACCENT),
        ("│                     │        │                     │", ACCENT),
        ("│  • ZMQ server       │        │  • Freq calculation │", ACCENT),
        ("│  • ML dipole calc   │        │  • Calls external   │", ACCENT),
        ("└─────────────────────┘        └─────────────────────┘", ACCENT),
        ("         ▲                                 │", DIM),
        ("         │                                 │", DIM),
        ("         │         ZMQ socket              │", DIM),
        ...
        ("                    │  • Bridge between processes │          │", ORANGE),
        ("                    │  • Sends file paths         │          │", ORANGE),
        ("                    │  • Waits for completion     │          │", ORANGE),
        ("                    └─────────────────────────────┘          │", ORANGE),
        ("                                                              │", DIM),
        ("                                                              └──▶", DIM),
        ("", TEXT),
