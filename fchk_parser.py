...
def convert_chk_to_fchk(chk_file: str, fchk_file: Optional[str] = None) -> str:
    ...
    try:
        ...
        raise RuntimeError("formchk command not found. Is Gaussian loaded?")
...
def get_fchk_from_chk(chk_file: str) -> str:
    ...
    if fchk_path.exists():
        if fchk_path.stat().st_mtime > chk_path.stat().st_mtime:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python fchk_parser.py <file.chk or file.fchk>")
        sys.exit(1)
