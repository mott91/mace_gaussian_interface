...
class SpectrumAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized analyzer: {freq_range[0]}-{freq_range[1]} cm^-1, "
    ...
    def extract_spectrum_from_fchk(self, fchk_path, log_path=None) -> SpectrumData:
        ...
        for i, line in enumerate(lines):
            ...
            if in_harmonic_section and (
                "Anharmonic" in line
                or "THERMOCHEMISTRY" in line
                or ("Temperature" in line and "Pressure" in line)
            ):
        ...
        if len(intensities) > n_modes:
            ...
            logger.warning(
                f"Only found {len(intensities)} IR intensities for {n_modes} modes, "
                "padding with zeros"
            )
    ...
    def match_by_mode(
        ...
        logger.info(
            f"Mode matching: {len(matched_modes)} matched, "
            f"{len(dft_only_modes)} DFT-only, {len(ml_only_modes)} ML-only "
            f"(match rate: {match_rate:.1%})"
        )

        return (
            np.array(dft_freq_matched),
            np.array(ml_freq_matched),
            np.array(dft_int_matched),
            np.array(ml_int_matched),
            match_stats,
        )
    ...
    def plot_combined_spectra(
        ...
        dft_norm = dft_broadened / dft_max if dft_max > 0 else dft_broadened
        ...
        for idx, (ml_spectrum, ml_name) in enumerate(zip(ml_spectra, ml_names)):
            ...
            ml_norm = ml_broadened / dft_max if dft_max > 0 else ml_broadened
    ...
    def plot_combined_regression(
        ...
        molecule_name: str | None = None,
        save_path: Optional[str] = None,
        mode_mappings: Optional[list[Optional[dict[int, int]]]] = None,
    ) -> plt.Figure:
        """
        ...
        for idx, (ml_spectrum, ml_name, metrics, mode_mapping) in enumerate(
            ...
            dft_freq, ml_freq, _, _, _ = self.match_by_mode(
                dft_spectrum, ml_spectrum, mode_mapping=mode_mapping
            )
            if len(dft_freq) == 0:
                continue
        ...
        for idx, (ml_spectrum, ml_name, metrics, mode_mapping) in enumerate(
            ...
            _, _, dft_int, ml_int, _ = self.match_by_mode(
                dft_spectrum, ml_spectrum, mode_mapping=mode_mapping
            )
            if len(dft_int) == 0:
                continue
